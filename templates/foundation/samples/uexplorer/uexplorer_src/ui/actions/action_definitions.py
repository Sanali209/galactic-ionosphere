...
def register_all_actions(registry: 'ActionRegistry', window: 'MainWindow'):
    ...
    if hasattr(window, 'dock_manager') and window.dock_manager:
...
def _sync_panel_states(registry: 'ActionRegistry', window: 'MainWindow'):
    ...
    panel_names = ["tags", "albums", "relations", "properties"]
    
    for name in panel_names:
        action_name = f"view.panel.{name}"
        # Panel visibility will be synced when dock_manager emits signals
        # For now, default to checked (visible)
        registry.set_checked(action_name, True)
