...
def bind(
    ...
    # Warn if signal missing and mode requires it for property change propagation
    if source_signal is None and mode in (BindingMode.ONE_WAY, BindingMode.TWO_WAY):
        logger.warning(
            f"Signal '{source_signal_name}' not found on {source.__class__.__name__}. "
            f"Property changes will not propagate to target. "
            f"Add '{source_signal_name} = Signal(...)' to your ViewModel."
        )
